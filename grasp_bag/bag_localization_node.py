...
class BagLocalizationServer(Node):
    def __init__(self):
        ...
        #self.sds_thread = threading.Thread(target=rclpy.spin, args=(self.sds_node, ), daemon=True)
    ...
    def range_to_angle(self, left_right, bag_dict):
        ...
        print(f"Angle to bag >>> {angle_to_bag}")
        return angle_to_bag
    ...
    def estimation_execute(self, srv_req, srv_res):
        ...
        bag_to_angle = self.range_to_angle(srv_req.left_right, bag_dict)
        if srv_req.graph:
            self.sds_node.graph_plot(srv_req.degree, self.search_range_data, bag_dict)
        srv_res.bag_to_angle = float(bag_to_angle)
        return srv_res
...
def main():
    ...
    executor.add_node(bls_node)
    executor.add_node(bls_node.sds_node)
    time.sleep(0.1)
    try:
        executor.spin()
